    ...
    for item in output {
        if item.get("type").and_then(|t| t.as_str()) == Some(MSG_TYPE) {
            if let Some(content) = item.get("content").and_then(|c| c.as_array()) {
                let (text, mentioned) =
                    extract_message_content(content, &mut citation_urls, &mut mentioned_brands, domain, company_name);
        ...
        if item.get("type").and_then(|t| t.as_str()) == Some(WEB_SEARCH_TYPE) {
            if let Some(action) = item.get("action") {
                if let Some(sources) = action.get("sources").and_then(|s| s.as_array()) {
                    your_product_mentioned =
                        your_product_mentioned || extract_web_search_sources(sources, &mut citation_urls, domain);
                }
            }
        }
    }
