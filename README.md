...
| `RUST_API_URL` | No | Rust API URL for the Bun server to proxy to (default: `http://127.0.0.1:3000`). **Required in production** if Rust runs in another process or host. On Railway, use private networking, e.g. `http://YourRustService.railway.internal:<PORT>` — **not** your Bun app’s `*.up.railway.app` URL (that causes proxy loops and multi‑minute hangs). |
